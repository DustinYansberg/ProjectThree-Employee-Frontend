...
;
  ...
  constructor(private appointmentService: AppointmentService, private messageService: MessageService, private userService: UserService, private employeeService: EmployeeService) {
    ...
    ngOnInit() {
      ...
      this.appointmentService.getAppointmentByAttendeeId(this.identityId)
        .subscribe({
          next: (response) => {
  ...
  checkIn(appointment: Appointment) {
    ...
    this.appointmentService.appointmentCheckIn(appointment.id).subscribe({
      next: (response) => {
        this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'Check-in successful!', life: 3000 });
